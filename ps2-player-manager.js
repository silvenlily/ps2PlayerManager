...
function startup() {
  ...
  db.connect()
    .then(() => {
      ...
      db.query("select * from information_schema.columns where table_name='users'")
        .then((res) => {
          let reqCol = {
            psname: "VARCHAR(32)",
            psid: "VARCHAR(19)",
          ...
          if (Object.keys(reqCol).length > 0) {
            for (let i = 0; i < Object.keys(reqCol).length; i++) {
              if (i === 0) {
                if (
                  Object.keys(reqCol)[i] === "psname" ||
                  Object.keys(reqCol)[i] === "psid" ||
                  Object.keys(reqCol)[i] === "discordid"
                ) {
                  qry =
                    qry +
                    "ADD COLUMN " +
                    Object.keys(reqCol)[i] +
                    " " +
                    reqCol[Object.keys(reqCol)[i]] +
                    " UNIQUE";
                } else {
                  qry =
                    qry +
            ...
            db.query("SELECT * FROM users")
              ...
              .catch((e) => {
                ...
                db.query("CREATE TABLE users ()")
                  ...
                  .catch(() => {
                    console.log(
                      "Unable to create table. Please create table 'users' then restart."
                    );
                    process.exit();
                  });
              });
        ...
        .catch((e) => {
          console.log(
            "Unable to query databases: information_schema.columns table, proceding assuming correct columns exist. This will likely cause bugs or a crash."
          );
          db.query("SELECT * FROM users")
            .then((res) => {
              console.log("Connected to database.");
  ...
  try {
    ...
    for (let i = 0; i < players.length; i++) {
      if (typeof dbCache[players[i]["name"]["first_lower"]] === "undefined") {
        console.log(
          `debug: adding user to db: ${players[i]["name"]["first_lower"]} id: ${players[i]["character_id"]}`
        );
        db.query("SELECT * FROM users WHERE psid = $1", [players[i]["character_id"]]).then(
          (res) => {}
        );
        db.query("INSERT INTO users(psname,psid,rank,status) VALUES ($1,$2,$3,$4);", [
          players[i]["name"]["first_lower"],
          players[i]["character_id"],
          players[i]["rank"],
          1,
        ]).catch((err) => {
          duplicates.push(players[i]);
        });
        dbCache[players[i]["name"]["first_lower"]] = {
          psname: players[i]["name"]["first_lower"],
          psid: players[i]["character_id"],
          rank: players[i]["rank"],
          status: 1,
        };
      } else if (dbCache[players[i]["name"]["first_lower"]]["rank"] != players[i]["rank"]) {
        db.query("UPDATE users SET rank = $1 WHERE psname = $2", [
          players[i]["rank"],
          players[i]["name"]["first_lower"],
        ]);
      ...
      if (lastlogin < inactiveDate) {
        if (dbCache[players[i]["name"]["first_lower"]]["status"] === 1) {
          ...
          db.query("UPDATE users SET inactiveranks = $1 WHERE psname = $2", [
            players[i]["rank"],
            players[i]["name"]["first_lower"],
          ]);
          db.query("UPDATE users SET status = $1 WHERE psname = $2", [
            2,
            players[i]["name"]["first_lower"],
          ]);
        }
...
bot.on("ready", () => {
  let guild = bot.guilds.find((g) => {
    if (g.id === config.dGuild) {
      console.log("Found discord guild");
      return true;
    }
  });
  if (
...
async function fixChanges() {
  ...
  for (let i = 0; i < guildMembers.length; i++) {
    log(
      7,
      "[" + (i + 1) + "/" + guildMembers.length + "] checking: " + guildMembers[i]["username"]
    );
    updateGuildMember(guildMembers[i]);
  }
  log(
    5,
    "Finished checking guild members, there are " +
      numMismatched.names +
      " users with mismatched names and " +
      numMismatched.ranks +
      " users with mismatched ranks."
  );
}

bot.on("messageCreate", async (msg) => {
  if (msg.content.substring(0, config.commandChar.length) === config.commandChar) {
      ...
      switch (command) {
        case "exempt":
          ...
          if (member.permission.has("administrator") || member.id === "229331045726552066") {
            ...
            sendTimedMessage(
              msg.channel.id,
              "You need to be an admistrator to use this command.",
              10
            );
          }
          msg.delete();
        ...
        case "urank":
          ...
          if (member.permission.has("manageRoles") || member.id === "229331045726552066") {
            bot.createMessage(
              msg.channel.id,
              "There are " + numMismatched.ranks + " users with mismatched roles."
            );
          } else {
            sendTimedMessage(
              msg.channel.id,
              "You need to have the manage roles permission to use this command.",
              10
            );
          }
          msg.delete();
        ...
        case "uname":
          ...
          if (member.permission.has("manageRoles") || member.id === "229331045726552066") {
            bot.createMessage(
              msg.channel.id,
              "There are " + numMismatched.names + " users with mismatched names."
            );
          } else {
            sendTimedMessage(
              msg.channel.id,
...
async function sendTimedMessage(channel, msg, time) {
  bot
    .createMessage(channel, msg)
    .then((newmsg) => {
      setTimeout(function () {
        newmsg.delete().catch((err) => console.log("delete timed message error: " + err));
      }, time * 1000);
    })
    ...
    if (!member.roles.includes(config["exempt"])) {
      if (!exemptMembers[member.id]) {
        if (!member.bot) {
          ...
          if (dbCache[playername]) {
            //if member exists in dbCache
            if (typeof dbCache[playername]["discordid"] != "string") {
              console.log("added discord id to db");
              try {
                db.query("UPDATE users SET discordid = $1 WHERE psname = $2", [
                  member.id,
                  playername,
                ]);
                ...
                console.log(
                  "Unable to add discord id: " +
                    playername +
                    ":" +
                    member.id +
                    " to database, likey caused by a username change."
                );
              }
            }
            if (member.roles.includes(config.unmached)) {
              //if member has bad ign role remove it
            ...
            if (!member.roles.includes(config.onLeave)) {
              if (dbCache[playername]["status"] === 2) {
                ...
                if (config.matchRanks) {
                  ...
                  if (member.roles.includes(config["ranks"][dbCache[playername]["rank"]])) {
            ...
            //if member does not exist in member cache
            numMismatched.names++;
            if (member.roles.includes(config.update)) {
              //if member has update role remove it
              member.removeRole(config.update);
            ...
            if (!member.roles.includes(config.unmached)) {
              //if member does not have bad ign role add it
